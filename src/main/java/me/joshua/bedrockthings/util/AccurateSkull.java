...
public class AccurateSkull extends AccurateProjectile{
	private static final DataParameter<Boolean> INVULNERABLE = EntityDataManager.createKey(AccurateSkull.class,
			DataSerializers.BOOLEAN);

	public AccurateSkull(EntityType<? extends AccurateSkull> p_i50147_1_, World p_i50147_2_) {
		super(p_i50147_1_, p_i50147_2_);
	}

	public AccurateSkull(World worldIn, LivingEntity shooter, double accelX, double accelY, double accelZ) {
		super(EntityInit.ACCURATE_SKULL.get(), shooter, accelX, accelY, accelZ, worldIn);
	}

	@OnlyIn(Dist.CLIENT)
	public AccurateSkull(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
		super(EntityInit.ACCURATE_SKULL.get(), x, y, z, accelX, accelY, accelZ, worldIn);
	}
	...
	public float getExplosionResistance(Explosion explosionIn, IBlockReader worldIn, BlockPos pos,
			BlockState blockStateIn, IFluidState p_180428_5_, float p_180428_6_) {
		return this.isSkullInvulnerable() && blockStateIn.canEntityDestroy(worldIn, pos, this)
				? Math.min(0.8F, p_180428_6_)
				: p_180428_6_;
	}
	...
	protected void onImpact(RayTraceResult result) {
		...
		if (!this.world.isRemote) {
			...
			Explosion.Mode explosion$mode = net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.world,
					this.shootingEntity) ? Explosion.Mode.DESTROY : Explosion.Mode.NONE;
